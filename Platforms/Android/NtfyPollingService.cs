...
namespace Widget.Platforms.Android
{
    ...
    public class NtfyPollingService : Service
    {
        ...
        {
            new NtfyChannel("alerts",       "🔴 Alerts"),
            new NtfyChannel("updates",      "🔵 Updates"),
            new NtfyChannel("errors",       "🟠 Errors"),
        ...
        private async Task PollAllChannels(HttpClient http, CancellationToken ct)
        {
            ...
            {
                var time = DateTimeOffset.FromUnixTimeSeconds(latestMessage.time).LocalDateTime.ToString("HH:mm");

                SaveLatestMessage(
                    label: summary,
                    title: $"[{latestChannel!.Label}] {latestMessage.title ?? ""}",
        ...
        private static void SaveLatestMessage(
            string label,
            string title,
            string message,
            string time)
        ...
        private void RefreshWidget()
        {
            ...
            var component = new ComponentName(
                                context,
                                Java.Lang.Class.FromType(typeof(WidgetProvider)));
